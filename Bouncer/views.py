...
def simple_upload(request):
    ...
    if request.method == 'POST' and len(request.FILES) == 0:
        ...
        for line in log_file.splitlines():
            ...
            if tokens != None:
                parsed_log = ParsedLog(owner = request.user, ip_address = tokens[0],rfc_id = tokens[1],user_id = tokens[2],date_time = tokens[3],request_line = tokens[4],http_status = tokens[5],num_bytes = tokens[6])
